...
- T-2.27 — Hit reactions on the rig: an optional `react(state)` layer on the rig contract; the skinned soldier's chest twists and leans away from where the round came from and the head snaps on a head-zone hit, scaled by the damage, recovering on the T-2.02 exponential and withdrawn bit-exactly; `effects.flinch` takes the hit's shooter, point and damage, works out the direction and the zone (`zoneAt`, `damage.json`), and keeps the T-2.11 translation flinch for a rig without a reaction; none while downed.

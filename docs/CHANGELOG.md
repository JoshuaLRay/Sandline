...
- T-2.34 prep — `docs/playtests/soldier-look.md` run sheet (not a verdict; says so) for the soldier-look sign-off: whether it reads as 2002 rather than as untextured geometry, the silhouette from every side, the look through E-2.2's and E-2.3's layers, telling six slots apart at 40 m, a firefight rather than a showroom, the grey box still rendering, and performance. Section 6 carries the one decision T-2.32 left open — the fixed low-resolution render target — because it trades crosshair and tracer legibility that T-2.24 and T-2.29 are judged on. `PLAN.md` §7.5 and the README now record the pass as agreed and built; the handoff doc is marked superseded and kept for its reasoning.

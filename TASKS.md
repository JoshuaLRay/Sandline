...
order. Its system tasks with no dependencies are open — the asset pipeline
(T-4.02), level format (T-4.09), mission sequence (T-4.14), reconnect
...
the rest: art sourcing (🧍 T-4.01) and where progress lives (🧍 T-4.21).
...
| Task | Status | Depends |
|---|---|---|
| 🧍 T-4.01 | OPEN — the owner's decision | — |
| T-4.02 | OPEN | — |
| T-4.03 | BLOCKED | T-4.02 |

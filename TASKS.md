...
**If nothing here is actionable** (no human available): there is no open
build task left in M2 right now — T-2.42 landed 2026-09-22 and every leaf
task through E-2.8 is done. Writing the three missing run sheets (`e2-3.md`,
`e2-5.md`, `e2-8.md`), modeled on `e2-2.md`/`soldier-look.md` and (for
`e2-8.md`) `e2-6.md` per T-2.43's own note, is real unblocked work an agent
can do — it does not require a human, only requires not inventing a verdict
inside it. Past that, **E-2.7** (combat audio, PLAN.md §7 epic table) needs
breaking out into leaf tasks and is blocked on an assets sourcing decision
(R1, §9 Q2 — purchased/commissioned/in-house).

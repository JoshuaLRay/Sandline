...
- **Size:** M
- **Completed 2026-09-26.** **Recipes** (`shared/src/audio/sounds.ts`, `data/audio/sounds.json`): a sound is a class, a length, a variant count, a peak level it is normalised to, BOUNDS on what its render must measure (peak and RMS in dBFS, seconds to the last sample above −60 dBFS), and 1–16 layers; a layer is a source (seeded `noise`, an `osc` — sine, triangle, square or saw — sweeping exponentially from `hz` to `toHz`, or an `impulse`), an attack/hold/decay envelope (`exp` falls 60 dB over the decay), biquads (low-pass, high-pass, band-pass, peaking; each may sweep its cutoff), a soft-clip `drive`, a feedback `delay`, a `reverb`, a gain, a start time, and per-variant `jitter` of pitch, gain and time. Validated by hand, unknown keys refused by name; in `shared` so the page can list what exists without the DSP. **DSP** (`tools/src/audio/dsp.ts`): the oscillators, noise from the shared `Sfc32`, the envelope, RBJ-cookbook biquads recomputed every 32 samples while sweeping, `tanh` soft clip, a feedback delay, and a reverb that is a generated impulse — seeded noise under a 60 dB decay through a one-pole low-pass closing along the tail — convolved by an in-house radix-2 FFT. **Render** (`render.ts`): each layer seeded from the sound's id, variant and layer index, placed at its start, mixed, peak-normalised, quantised to 16-bit (held at full scale, never wrapped) and written as mono **WAV at 48 kHz**: every target browser decodes 16-bit PCM WAV, it needs no encoder dependency, and it is byte-identical across machines, which a codec's output is not promised to be; one-shots are ~96 KB a second, and a lossy format is the next change if the set outgrows a few megabytes. **`pnpm gen:audio`** writes `client/public/audio/<id>.<variant>.wav`, removes renders of recipes that are gone, and commits `renders.json` — the hash of the recipes' parsed content and of the DSP and render source text, and each file's size, digest and measured numbers (the sound board, T-2.45, shows them) — and exits 1 naming any render outside its bounds. One placeholder, the **click** (band-passed noise over a falling sine, two variants), proves the path. **Tests:** `shared/audio/sounds.test.ts` (the file, the defaults, every refusal by name) and `tools/audio/audio.test.ts` (a low-pass and a high-pass on tones either side of the cutoff; the envelope's −60 dB point; FFT convolution against the direct sum; 16-bit holding and a WAV round trip; a render byte-identical when repeated and variants differing; every committed file what gen:audio wrote and what the live recipes make; the inputs hash moving with a recipe and with the DSP source; every render within its bounds and unclipped). No audio device anywhere.
...
- **Size:** M
- **Completed 2026-09-26.** **The mix, as data** (`data/audio/mix.json`, `shared/audio/mix.ts`): per sound class a priority and a falloff curve — `[metres, gain]` points, linear between, held past the ends, null for the unplaced UI class; the voice limit (24) and who keeps a voice (`ownBonus` for your own sound, `perMetre` off for distance); occlusion as a low-pass (900 Hz) and a cut (−9 dB); the speed of sound (343 m/s). Validated by hand, refusals by name; `falloffGain`, `voicePriority` and `soundDelaySeconds` are pure. **Placement** (`client/src/audio/spatial.ts`): `occlusion` casts `rayWorld` from the ear to the source through the world's boxes (stopping 5 cm short, so a source on a face is not behind it) and gives the mix's low-pass and cut on a hit and nothing on a clear line; `place` combines falloff, occlusion, the sound's delay (none for your own) and its priority; `VoicePool` keeps to the limit, stealing the least important voice (the oldest of equals) for a sound that outranks it and dropping one that outranks nobody. **The engine** (`engine.ts`) is written against the few Web Audio calls it makes, so a test drives it with a fake context: the context is made on the first click or key (autoplay rules) and every render fetched and decoded then; the listener follows the camera's position and view direction; a placed sound goes source → (low-pass when occluded) → gain → HRTF panner at the source → the effects or voice bus, started late by distance over the speed of sound; a UI sound goes straight to its bus; variants cycle, so automatic fire never repeats one back to back; master, effects and voice gains take the three volumes the settings have held since T-4.26 (the pause menu's Settings tab is the control). **`?sounds`** is the sound board (`ui/SoundBoard.ts`, over the menus): every committed sound, each variant a play button beside its waveform (drawn from the WAV itself) and the numbers `gen:audio` measured from `renders.json`; a recipe with no render is listed as such, never hidden. **Tests:** `shared/audio/mix.test.ts` (interpolation and holding, priority, delay, refusals), `client/audio/spatial.test.ts` (a wall between gives the attenuation, a clear line, a face and over-the-top none; placement's gain, filter and delay by distance, none for your own; stealing and dropping under the limit), `client/audio/engine.test.ts` (headless: nothing before unlock, every variant loaded on it, the UI and placed routings, the HRTF panner at the source, a muffled and delayed distant shot behind a wall, a nearer shot stealing a farther one and a farther one dropped, a finished voice freeing, variants never repeating, the three volumes) and `ui/soundBoardModel.test.ts` (every committed sound listed with its measured numbers, a missing render noted, a committed WAV read and drawn). Smoke-tested in Chromium: the board lists and draws the click and plays both variants. Only the click exists to hear until T-2.46 and T-2.47 add the game's sounds.
...
- **Done when:**
  ...
  - render bounds hold.
- **Size:** M
- **Completed 2026-09-26.** **Recipes** (29 more renders, all in bounds; the audio set is now 6.6 MB, fetched once when the audio unlocks): rounds going past (`crack`, a driven high-passed snap with a short room; `whiz`, a band-pass sweeping down), impacts (`impact-wall` thud and grit, `impact-crate` a hollow wooden knock, `impact-post` a ringing metal ping, `impact-ground` a dirt puff), the grenade (`grenade-throw` whoosh and pin, `grenade-bounce` clunk), the rocket (`rocket-launch`, `rocket-flight`), explosions (`explosion-near` a driven, sweeping blast over a falling sub thump and a long room; `explosion-far` a muffled low boom with a longer, darker tail), footsteps by stance (`step-walk`, `step-run`, `step-crouch`, `step-prone`, four variants each), the `vault` and the `land`, and bodies (`body-hit`, `downed`, `body-fall`). **Mapping** (`data/audio/worldSounds.json`, `shared/audio/worldSounds.ts`, every name checked): the near-miss distance (3 m), impacts by surface, the explosion's far distance (60 m), footsteps by stance, the landing's minimum airtime. **In play** (`client/audio/worldSounds.ts`, `main.ts`, and a gait callback on `RemoteSoldiers`): footsteps land on the gait's heel strikes — the stride's extremes, π/2 for the right foot and 3π/2 for the left, where the pose driver has the leading heel down — for your own soldier and every remote one, from the same phase the body walks by (`FootstepTracker`), and none standing still or mid-vault; a round from another shooter whose path passes within 3 m of your head plays its crack and whiz at the path's nearest point, so it sounds on the side it went by, and a round that stopped short plays none (`nearMissAt`); an impact plays by what it struck (posts and rails metal, crates and drums wood, other boxes wall, open ground ground); a round into a soldier thuds; a blast plays near or far by its distance from the ear; a new projectile plays the grenade's throw or the rocket's launch and flight, and a grenade's upward bounce its clunk; your own vault and a landing after 0.35 s in the air; any soldier going down or dying where they lie. The rocket's flight is a one-shot 1.8 s render played at launch, not a loop following the rocket: flights are short, and a moving looped source is the change to make if it reads wrong. **Tests:** `shared/audio/worldSounds.test.ts` (names and classes; refusals) and `client/audio/worldSounds.test.ts` (footsteps checked against the real pose driver's phase over ten seconds of walking: every step in the frame its heel strike was crossed, feet alternating, two a cycle; none idle or vaulting; a near miss 1 m to either side cracks on that side, a wide or short round plays none; the explosion's version by distance; impacts by surface), with the renders' bounds held by T-2.44's test. Smoke-tested in Chromium on the range with `?enemies`: walking and sprinting steps (yours and the patrols'), a landing, impacts on ground, wall and post, cracks and whizzes from the riflemen's fire, the grenade's throw, bounce and blast, body hits and a soldier going down.
...
- **Size:** M
- **Completed 2026-09-26** — the pipeline, tested on generated signals; **no recordings are uploaded yet**, so no lines are committed and the sound board says so. **Data** (`data/audio/voices.json`, `shared/audio/voices.ts`, validated by hand, refusals by name): the script as `docs/audio/voice-script.md` asks for it — ten sections, one file per pass (`<section>-normal`, `-shout`, and `hit-hurt`, which is the three pain sounds only; the script now says so), each line said three times — and which passes become game lines (shouted for contact, firing, moving, reloading, grenades and hits; the hurt pain sounds; normal for revives, kills, orders and the objective); how takes are split; the loudness target (−16 LUFS, ±0.5 LU, ceiling −1 dBFS); **six profiles, one a slot** (pitch and formant semitones apart, an EQ, a shout's drive, and which speaker, default the slot modulo the speakers there are); the radio (band-pass 350–3200 Hz, drive, hiss at −36 dB, and two new recipes, `radio-in` and `radio-out`, for the squelch) and the distance treatment (dulled to 1.6 kHz, 1.4 s room), of which `dry` and `radio` are rendered; two variants a line, each from the next take and a seeded pitch nudge of up to 0.35 semitones; 24 kHz output. **The formant-preserving method is TD-PSOLA, in TypeScript in `tools`** (no Python and no new dependency): a YIN pitch track on a 12 kHz copy, pitch marks on each pulse, and two-period Hann grains laid down at the new pulse spacing, each resampled (Blackman-windowed sinc) to move the formants on their own; unvoiced stretches go through at 5 ms spacing. **Pipeline** (`tools/src/voice/`, `pnpm gen:voice`): a speaker folder without its `CONSENT.md` is refused whole, and a file the script does not name is refused by name; WAV is read in any common shape (8–32-bit integer or float, extensible, any rate, channels mixed down), anything else through `ffmpeg` if installed, else refused with what to do; rumble is high-passed, the pass split (frames within 35 dB of the loudest, runs closer than 600 ms joined, shorter than 120 ms dropped, padded), and the takes named by position — a count that does not match the script fails, listing where each take starts, and `edits.json` can drop a false start by index; each line is voiced in its profile, treated, trimmed, brought to its loudness (BS.1770-4 integrated loudness: K-weighting, 400 ms blocks, both gates; gain, then a soft-knee limiter when the peak would pass the ceiling, repeated until it lands) and written as 16-bit WAV to `client/public/audio/voice/<profile>/<line>.<style>.<treatment>.<variant>.wav`, with `renders.json` holding the hash of the uploads' bytes, the data, the squelch recipes and the pipeline's source, and each line's loudness, peak, length and digest. **Size, when recordings come:** ~45 lines in their rendered passes × 6 profiles × 2 variants × 2 treatments ≈ 1,100 files of about a second at 48 KB/s — around 50 MB, over the few megabytes WAV was chosen for (T-2.44): encoding the voice lines lossily is the change to make with the first upload, before they join the initial pack. **Tests:** `tools/voice/voice.test.ts` — a band-limited resample; BS.1770 on a 1 kHz sine (−20 dBFS peak → −23 LUFS); quiet, loud, short and plosive lines brought within ±0.5 LU under the ceiling; the splitter finding all seven takes of a file of tones and silences (quiet, loud, short, and one with a 250 ms pause inside) and dropping a click; takes named by position, a false start refused and then dropped by an edit; **on a synthetic vowel (a pulse train through formant resonators at 700, 1220 and 2600 Hz), four profiles each move the fundamental within 1 % of their amount (measured: 95.24 for 95.25 Hz, 142.71 for 142.71, 80.13 for 80.09) with every formant within a semitone of its own shift**, while resampling — the slowed-tape way — moves F1 by more than a semitone and fails that check; a 24-bit stereo 44.1 kHz WAV read as mono; consent and names enforced; a whole run on a synthetic speaker making every line for every profile and treatment on target, the two profiles two semitones apart, and the same bytes twice; and the committed state — its hash what the uploads, data and source make now, every listed file as written and nothing else, and an upload added without re-processing changing the hash (the README does not). `shared/audio/voices.test.ts` checks the data and its refusals; the sound board lists the voice lines, or says nobody has recorded yet.
...
- **Size:** S
- **Completed 2026-09-26** — wired, and heard as the **radio chirp** (`radio-chirp`, a new recipe: a key click, two falling beeps and hiss) until someone records the voice script; the recorded lines drop in with `pnpm gen:voice` and no code change. **Data** (`data/audio/callouts.json`, `shared/audio/callouts.ts`, every line checked against what the voice pipeline renders, with the pass it comes from): twenty events — contact (and the MG by name), reloading, frag out, a grenade warning, I'm hit, I'm down, man down, reviving, you're up, enemy down, the five order acknowledgements and can't-get-there, and the three objective lines — each with its lines (said in turn), a cooldown scoped to the squad or to the speaker, a priority, and whether your own soldier says it; the radio distance (25 m), queue (1.5 s) and gap; the speaker gap; when a sighting is new (seen within 70 m, nobody having seen it for 25 s, checked four times a second); the grenade-warning distance (8 m); the kill-credit window (1.5 s). **The director** (`client/audio/callouts.ts`, pure): an event is not said again within its cooldown, and a soldier never starts a line while saying one (plus a gap); your own soldier is heard as your own, a squadmate within 25 m where they stand, dry, and one beyond over the radio treatment, unplaced; one voice on the radio at a time — the next waits, the most urgent first, and is dropped when stale; each slot speaks with its profile, variants in turn, and a line with no recording is the chirp. **The watcher** (`client/audio/calloutEvents.ts`) finds the events in what the client has: a clear line from a squadmate's eye to an enemy's chest (`rayWorld` through the world's boxes) for a new contact — the call is a squadmate's, never yours for what you see; a reload starting; a squad grenade in the air, and anyone else's within 8 m of a squadmate; a round into a squadmate (the server's shot event), one going down and the nearest other's "man down"; a reviver starting and the one they picked up standing; an enemy dying within 1.5 s of a squadmate's round or blast (the blast's thrower from its projectile, kept after the entity goes); a bot's order changing; the objective clear, half held, and secured; and a mission script's callout id when it names an event. **Protocol 33:** the host now says when a bot's order fails (`OrderFailed {slot, order}`, an event variant beside the script callout), which is how a bot says it can't get there — before, a failure only took the order off the list, indistinguishable from finishing it. The engine gained `playFile` (a voice line fetched and decoded on first use, on the voice bus, false rather than an error when it cannot be had — the chirp then), and `NetClient` the reviver of a remote. **Tests:** `client/audio/callouts.test.ts` (the director: once within the squad's and a speaker's cooldown; never two lines at once from one soldier; near and far and your own; one voice on the radio, the most urgent waiting line first, a stale one dropped, a near line not waiting on the radio; a recorded line and its variants when there is one, the chirp with no error when there is not; the engine playing a voice file and resolving false for a missing one — and the watcher: a contact not through a wall, not twice while seen, the MG by name, new again after being forgotten; a reload; frag out; a grenade warning once; hit, down and man down; reviving and revived; enemy down to the right squadmate by round and by blast and to nobody after too long; order acknowledgements, can't-get-there, the three objective lines, and a script's callout), `shared/audio/callouts.test.ts` (the data and its refusals), `shared/net/protocol.test.ts` (OrderFailed round-trips) and `server/ai/friendly/orders.test.ts` (an unreachable move tells the lead; a finished one does not). Smoke-tested in Chromium on the range with `?squad&enemies`: your own reload and grenade called, and the squad's fight with the riflemen calling as it went, no page errors. Whether they help or nag is T-2.50's question.
...
- **Size:** S
- **Completed 2026-09-24.** Missions may add a deterministic `failure`
  rule for a time limit and/or one protected encounter entity; the existing
  squad-wipe rule remains universal. Completing an objective records a
  checkpoint: squad positions, elapsed mission time, cleared encounter groups,
  and T-4.15's scripted-event state. A failed `MissionRestart` restores that
  state, resumes the current objective, and does not resurrect cleared groups
  or replay already-fired events. A completed mission still takes the full
  restart path to objective zero and the original spawn line. Session tests
  cover both new failure rules, checkpoint retry with earlier work preserved,
  and the distinct full restart.
...
- **Size:** S
- **Completed 2026-09-26.** **Data:** `shared/src/data/regions.json` and `sim/regions.ts` — a region is an id, Fly's region, a name, a one-letter voice-safe tag and a bare `wss://` address, validated by hand with unknown keys, a bad tag, a twice-used tag or id and a `ws://` host refused by name. One region today, `na-east` on the deployed host; the addendum's rule says when the second comes. **Tagged codes:** `tagCode('A', 'KM7X')` is `A-KM7X`, and `splitTaggedCode` takes a typed or pasted code apart — forgiving case and separators, a tag only when a region has it, five or nine characters otherwise not a code — so the wire never sees a tag: a host is one region's. **The lobby** (`ui/Lobby.ts`) has a Region select above the host field: every region's `/healthz` is pinged once when the lobby is made (`client/net/regions.ts`: `measureRtt`, one timed fetch, null when it fails, answers badly or takes 3 s) and the round trip shown beside its name; `pickRegion` is pure — the remembered choice (`sandline.region`) wins whatever the numbers, else the lowest that answered, else the first — and the choice puts the region's address in the host field, following the pings as they come in only while nobody has chosen or typed; a typed host stands on its own. **A code from another region:** `A-KM7X` in the code field switches the host to region A's and joins there whatever the player's region; the share link and the room lobby's Copy link carry the tag when the host is a region's (`taggedRoom` in `main.ts`), so an invite lands in the right one; an untagged code is the region the player is on, which is every code from before this task and a laptop's. **Tests:** `shared/sim/regions.test.ts` (the file and its refusals; tagging and splitting; a region's host) and `client/net/regions.test.ts` (the choice from fake round trips, the remembered override, a stale memory, nothing answering; a code from another region resolving to it; the health URL, the timing, and the ping's failure modes). The lobby's DOM is not unit-tested; it was smoke-tested in the page.
...
- **Size:** M
- **Completed 2026-09-26.** `packages/client/src/ui/hud/`: `hudModel.ts` is the HUD as pure functions of replicated state (each widget a function of its inputs, tested without a page in `hudModel.test.ts`), and `Hud.ts` puts what they return into elements, touching the DOM only on change. Vitals (the host's health, or the bleed-out and reviver while downed, the respawn while dead, toned by how much is left), the magazine and the pouch from the predicted weapon state (`CombatQA.magazine`, `ThrowQA.rows`; low at a quarter, empty, the reload as a bar), the stance from what the body is doing (downed, vault, airborne, prone, crouched, standing, in that order), the objective line (`missionLine`, now on the HUD alone; `#mission` keeps the after-action credit and script notices), six squad rows always (name or Bot, the host's vitality per slot, the order each bot is under), a compass strip (north +Z, east -X, the soldier's right; the T-3.29 order and mark markers by bearing and distance, within ±90° of the look), a hit marker on the server's word that our round landed on a soldier, and a damage direction arc for each round or blast that hit us, from the shooter's or the blast's replicated position. The QA readouts start folded behind H, and the tuning panels fold with them (the squad panel and the netgraph, N, stay). Chromium: joined an in-page session with enemies and the mission, the HUD read 100 / STANDING / MK4 Carbine 30 / 30 / the pouch / the objective / six rows / heading 0° with N-NE-E-W-NW, crouching read CROUCHED, a burst read 20 / 30, enemy fire dropped the bar to 26 in critical red with downed and dead rows toned, and a damage arc showed. The deployed site is checked after the merge, as the Pages workflow deploys from main.
...
- **Size:** S
- **Completed 2026-09-26.** `packages/client/src/ui/menu/`: `Menu.ts` is the main menu — the lobby unchanged as its Play panel, restyled into the menu's card — the pause menu Esc opens in a session (Resume, Settings, Leave; the session runs on, Esc again resumes and takes the mouse back), and the settings panel both share: mouse sensitivity, invert, field of view, three audio volumes kept for E-2.7 and saying so, a graphics quality that names a row of ADR-013's knobs (`settings.ts`: `medium` is the budget itself — a 1.5 pixel-ratio cap and a 2048 shadow map — `low` under it, `high` a discrete GPU's headroom on resolution and shadows, never geometry), and the key bindings shown, not yet rebindable. Every change is applied by the page at once (`LocalInput`, the camera's base field of view, the renderer's pixel ratio and the sun's shadow map) and stored per browser under `sandline.settings`; parsing is tolerant, so an older or hand-edited value falls back field by field, and a refused store costs the settings, not the game. `settings.test.ts` covers the parsing and the store; `menu.browser.test.ts` runs the flows in headless Chromium under the `assets-browsers` project (the main menu's tabs and the lobby's own hide, the pause menu's Resume, first tab and Leave, and a setting changed, stored, back in a fresh menu from the same store, and reset), and a page-level run showed the menu at boot, a field of view and quality set and back after a reload, Esc pausing an in-page session, Resume, and Leave to the main menu.
...
- **Size:** M
- **Completed 2026-09-26.** `data/classes.json` and `shared/sim/classes.ts`: each class is a loadout (weapons.json ids, the first in hand at spawn; the pouch it spawns with), a health, and whom its orders reach (`squad` or its own `fireteam`); `slotDefaults` gives the class a slot takes with no pick (the assault fireteam leads, overwatch watches) and `required` what the squad must not lack. `assignClasses` is pure: a human's pick stands, a slot without one takes its default, and bots fill the required shortfall lowest slot first (three humans all Marksmen leave a bot in overwatch leading; one taking the lead back returns the bot to its default). `orderReach` is the order rule, and the session applies it in `orderFrom`: a Team Leader's order reaches the squad, a Marksman's only their own fireteam, whoever the address named. The room's `class` command (protocol 30) carries a pick; `RoomState.classes` and a new `RosterEntry.classId` replicate the assignment to every client, so the room lobby names each slot's class with a picker for your own and the T-4.25 HUD's squad rows show the class's two letters. A session with a room lobby plays by loadouts (`SessionOptions.loadouts`, 'class'): at assignment, restart and respawn a slot holds its class's first gun, pouch and health (the Marksman's 90), and an Equip outside its guns is refused; the in-page session and a plain QA host stay 'free', so the range and the tuning panels keep every gun — classes still decide who may order whom there. The M203 stays unbuilt: no loadout item fires one yet. Tests: `classes.test.ts` (data refusals, assignment and fill, reach), `session/classes.test.ts` (pick, replication and fill over loopback; loadout, health and the refused Equip at the start; the free session; a Team Leader's and a Marksman's order reaching bots), and the protocol round-trips.
...
- **Size:** S
- **Completed 2026-09-26.** The numbers are the server's: the session counts each slot's kills (an enemy killed by its round or its blast), deaths (a finishing shot, a blast, or bleeding out), revives, orders given (one a bot took) and orders carried (a bot's order reported done) as they happen, and sends the six rows whole in a `Stats` message (protocol 31, `shared/sim/scoreboard.ts`) on every change, on seating, every five seconds while a mission runs (so the clock keeps up), and when the mission's state or objective moves, with the mission clock (`MissionRun.elapsed`) and the objectives done; a restart starts a new board, a checkpoint retry keeps it. The page holds the scoreboard up while Tab is down (`ui/scoreboard.ts`; Tab never moves focus in a session) and on its own once the mission is over, six rows always with the roster's names, and the `#mission` line gains the after-action summary — how it ended, in how long, the objectives done — over the T-4.24 credit. `scoreboardRows` and `afterActionSummary` are pure and tested; `session/scoreboard.test.ts` counts a death under an enemy's fire and the bleed-out after it (once, kept across the respawn), a kill reaching every client, a revive, and an order a bot took against one nobody took, and round-trips the message. Chromium: Tab over the in-page mission showed the six named rows and the clock, and hid on release.
...
- **Size:** M
- **Completed 2026-09-26.** Protocol 32. **Data** (`shared/src/data/emplacements.json`, `sim/emplacement.ts`): one kind, `mg-nest` — the LMG's `weapons.json` row, a ±60° traverse, −10°/+20° elevation, a 1.6 m mount range, the gunner 0.7 m behind the gun and the rounds leaving 1.2 m up; heat 0..1 (0.02 a round, −0.1/s, overheated at 1 until below 0.4); and the AI's `takeWithinM` (4 m) and `leaveAfterSeconds` (3). Validated by hand, unknown keys refused by name, and the arc/elevation clamps and the heat curve are pure functions in wire angle units. **Placement:** a world or level file's `emplacements` list (`World.emplacements`, id, kind, place, `yawDeg`), validated with the world; mission-01 has one inside the compound facing the west gap, a 0.9 m sandbag in front of it (the navmesh re-baked). The range has none, deliberately: the AI scenarios are calibrated on it. **The wire:** an `Emplacement` component (id 11: kind, gunner — nobody, a slot + 1, or 7 for an enemy — heat as whole percent, overheated) on an entity whose Transform is the gun's place and the way it is laid; netIds after the slots'. **Session:** a press of E (the edge, not the hold) within the mount range of an empty gun's gunner's place mounts — the revive has first claim on the same press — and a second press dismounts; the gunner is teleported to the place and held there crouched with no movement, looking within the arc (`pinGunner` on every tick and on every Input); a mounted `Fire` fires the gun's weapon and belt from the gun's muzzle through the one `traceShot`, aimed always, the aim held within the arc and elevation, refused while overheated, warming the gun a round at a time; the guns cool and their bloom settles every tick; an `Equip` while mounted is ignored. A downed, dead or departed gunner is off the gun, and a restart frees, cools and re-belts every gun. **The AI's use is the same path:** `aiShoot` takes an optional gun and then uses its weapon, belt and muzzle, holds the aim within the arc, adds heat and stops when overheated; an enemy carrying the gun's weapon (the MG) within `takeWithinM` of an empty gun takes it when the gun can bear on what it is fighting, is deployed at once, is pinned at the place while its brain's feet are ignored, and leaves it after its target has been outside the arc for the data's seconds, or dies. **Page:** `NetClient` reads the component whole (`emplacements()`, `mounted`); on a gun the page sends the same input the host holds the gunner to (no movement, crouched, yaw clamped) so prediction and authority agree, `LocalInput.setViewLimits` stops the mouse at the arc and the elevation, `CombatQA.useGun` runs the trigger, cone and HUD on the LMG (index −1: nothing is equipped or named), and the loadout gun returns on dismount; a code-built tripod with the enemy's LMG model on the pintle is drawn at every gun, laid the way the host says; the HUD shows `E  MAN THE GUN` within reach of an empty gun, `E  LEAVE THE GUN` on one, and a heat bar (HEAT / HOT / OVERHEATED) under the magazine. **Not here:** the remote gunner's body is drawn as it stands (crouched, rifle in hand) rather than posed on the gun; the predicted tracers of a mounted gun use the page's own shot index, so they and the host's rounds share the cone but not the seed; the deployed-site check is the M4 gate's (T-4.34). **Tests:** `shared/sim/emplacement.test.ts` (the table and its refusals; the arc across the wrap; the elevation; heat rising, overheating, cooling and firing again; the gunner's place and muzzle; a world file's list validated) and `server/session/emplacement.test.ts` (a press mounts and a hold does not re-press; a press from afar does nothing; a gunner moves nothing and the gun follows the look to the stops; a mounted Fire is the LMG from the muzzle, on a man at 15 m, warming the gun, and a shot aimed 90° off leaves along the 60° stop; a belt fired without pause overheats and fires again below the threshold; the sandbag stops chest shots and head shots land; an MG takes the gun, fires from its muzzle and leaves it 3 s after its target goes behind it; a downed gunner, a departed one and a restart free the gun), plus `heatView` in `hudModel.test.ts`. Smoke-tested in the page on mission-01: walk to the nest, mount, the view stops at the arc, fire, the heat bar climbs, dismount.
...
- **Size:** S
- **Addendum written 2026-09-26; the owner's agreement of the cost is outstanding.** `docs/adr/011-regional-session-hosting.md`, second addendum. **Regions:** a data file (`regions.json`: id, Fly region, name, a one-letter voice-safe tag, address); the launch set is `iad` now and `lhr` when a European party needs it, a region added when a regular player measures over 80 ms to every one that exists. **Allocation:** codes carry their region as a tag (`A-KM7X`; campaign codes `A-KM7XRT34`, bound to their region's volume and never moved); the client goes straight to the code's region, or to its lowest-RTT region to create; one Fly app per region; within a region the hosts find each other over `<app>.internal` and answer a Join for a room a peer holds with `fly-replay` (a `redirect` disconnect code as the portable fallback), so T-4.31's allocator is a library in the host with fake peers in its tests and **no allocator machine**. Worst-case-RTT matching is made explicit (the room lobby shows every member their RTT to the room's region) rather than computed. **Machines:** one per region, stopped whenever nobody is connected, a second added by hand when `host full` refusals show in T-4.33's metrics; nothing kept warm. **Cost:** ≈ $0.50 a month idle per region (a stopped machine's root filesystem and a 1 GB volume), ≈ $0.01 an hour of play per machine plus ≈ $0.01 an hour per full room of bandwidth; two regions and a playtest a week is a few dollars a month; a region kept warm is ≈ $3.19 a month more. Fly's list prices, to be checked against fly.io/pricing when agreed. What the owner is asked to agree is spelled out in the addendum's last paragraph.
...
- **Size:** M
- **Built 2026-09-26; the two-region deploy is the owner's.** As the addendum decided: a library in every host, no allocator machine. `server/src/allocator/Allocator.ts` answers two questions before a socket is upgraded, from the code in the upgrade URL (`?room=`, which `RemoteServer.socketUrlFor` now puts there and `setRoom` keeps there once seated, so a reconnect lands on the machine that holds the room): **who holds a code** — this host (a live room or a saved campaign), else the first peer answering 200 to `/internal/room/<code>`, else nobody and the host takes the connection so the handshake can say `no such room`; **who takes a new room** — the least-loaded of this host and every peer that answers `/healthz`, is not draining and has a room to spare, ties to this host. Peers are asked in parallel within `timeoutMs` (500 ms); one that does not answer, or answers badly, is skipped for that decision and logged. `flyPeers.ts` discovers this region's other machines from Fly's `vms.<app>.internal` TXT record (`<id> <region>` pairs), naming each `http://<id>.vm.<app>.internal:<port>`; `flyEnvironment` reads `FLY_APP_NAME`/`FLY_REGION`/`FLY_MACHINE_ID`, and without them a host is alone. `WsTransport.startWsServer`'s `shouldAccept` may now be async and answer a **replay**: the upgrade is answered `409` with `fly-replay: instance=<id>` and closed, which Fly's proxy re-sends to that machine. `SessionHost` builds the allocator from `options.allocator`, answers `/internal/room/<code>` only from the private network (`fdaa::/16` and loopback; 404 to anyone else, so codes stay unlisted), and reports `instance`, `region` and `draining` in `/healthz`. **Tests:** `allocator/Allocator.test.ts` with fake peers (here / a peer / nobody; least loaded, ties, full and draining skipped; a dead, a slow and a nonsense peer skipped and named; discovery failing; the URL, the private-address rule and Fly's TXT parsing) and `session/allocator.test.ts` on two real hosts over loopback (an upgrade for the other's room answered 409 with the replay header; a new room sent to the emptier host and a tie kept; `/internal/room` yes/no to a peer and 404 from a host alone; a stopped peer skipped). **Owed:** the two-region deploy and, with it, the spike that Fly honours `fly-replay` on a WebSocket upgrade (DEPLOYING.md says what to try); the `redirect` fallback is next if it does not.
...
- **Size:** M
- **Built 2026-09-26; the staging run is the owner's.** `SessionHost.drain(capMs)`: the host stops taking new rooms — the upgrade and the handshake both answer `host draining` to a create, a quick-join or a code it does not hold, and `/healthz` answers 503 so the platform sends new connections elsewhere — but keeps every room it holds and still admits a code for one (a reconnect, a friend's late join); it stops the moment nobody is seated, or at the cap, telling the players first (`host draining` with a reason that says the deploy could not wait), and the promise resolves once the socket is closed. `main.ts` drains on SIGTERM/SIGINT with `DRAIN_MAX_MS` (default 270 s) and `fly.toml` gives the machine `kill_timeout = "300s"`, Fly's ceiling: **a mission longer than five minutes is cut short at a deploy**, with the players told — the accepted limit until the platform allows more, and why the Host workflow stays on demand for a playtest evening. The new version takes the new rooms because the old machine's health check fails while it drains and the allocator (T-4.31) places nothing on a draining peer. **Tests** (`session/drain.test.ts`, fake clock): a host with nobody seated stops at once; one with a room in progress keeps it, refuses a new room, admits a friend's code and a dropped player's resume, and stops the tick after the last player leaves; one at the cap stops there and not a tick sooner, with every player told. **Owed:** the staging run — a deploy with a room in progress kept to the end — which needs the Fly account.
...
- **Size:** S
- **Completed 2026-09-26.** `server/src/metrics.ts` is a hand-rolled Prometheus text exposition — a counter, a gauge and one histogram are all it needs, and a library would be the only reason to think about it again. `HostMetrics` counts what only the host sees as it happens: every host tick's wall time (`sandline_tick_seconds`, buckets up to and past the 33.3 ms period; `sandline_tick_overruns_total` for the ones over it; `sandline_ticks_dropped_total` from the clock) and every typed refusal or drop (`sandline_refusals_total{code}`, from a new `ServerConnection.rejectedWith` read when the socket closes, so a refusal from the host's routing, a room's `room full` and a handshake's `bad version` all count the same way; a peer that left is not one). What a room counts — bytes and snapshots sent, seatings fresh and resumed (`SessionStats.joins`/`resumes`, T-4.18's reconnects), and the AI's milliseconds (`SessionStats.aiMs`, T-3.35's `profileAi`, now on for every room of a host with `HOST_AI=1`) — is summed over the live rooms plus a running total of the reclaimed ones (`retire` on the registry's reclaim hook), so no counter ever goes down when a room does. Gauges from the registry: rooms, idle rooms and the cap, players, connections and their cap, the process's RSS, and `sandline_info{protocol,draining}`. **The endpoint** is `/metrics` on the host's port beside `/healthz`; room codes are in neither. `fly.toml`'s `[metrics]` block has Fly's managed Prometheus scrape it. **The dashboard** is `docs/observability/grafana-dashboard.json` (tick p50/p95/p99 against the period, the AI's share, overruns and drops, memory; players, rooms and connections against their caps; bytes per player against ADR-012's 18 KB/s; seatings and reconnects; refusals by code) and **the alerts** `docs/observability/alerts.yml` (Grafana provisioning format: *Ticks overrunning* — p99 over the period for 2 min; *Ticks dropped* — any in 5 min; *Refusals* — a run of `host full`/`room full`, the addendum's signal for another machine), with `docs/observability/README.md` saying what every metric means and how to import both into fly-metrics.net, which takes no provisioning files. **Tests:** `metrics.test.ts` (the exposition's shape, the cumulative buckets and overruns, a reclaimed room's counters kept, refusals by code sorted and escaped) and `session/metricsEndpoint.test.ts` (a real host serves `/metrics` over HTTP with the registry's gauges and the ticks it ran; `no such room` and `host full` counted as they happen; a resume counted as a reconnect and not a refusal; a reclaimed room's seatings and bytes kept). The dashboard and alerts are written, not yet imported into the org's Grafana: that needs the account, and is a five-minute job noted in DEPLOYING.md.

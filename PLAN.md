...
- **Size:** M
- **Completed 2026-09-20.** Button bit 8 (`E`, held). `Session.resolveRevives` runs after movement and before the snapshot: continue-or-cancel what is under way, then start on the nearest unattended downed teammate in reach; one reviver per downed soldier. `Health` carries the revive progress (fifteenths) and the reviver's SEAT (7 = nobody), and the snapshot now carries `PlayerSlot`, so a client can name anyone from the roster (protocol v8). `encodeVitals`/`decodeVitals` are the two ends of the field list. Both prompts are on the HUD: the downed player's banner names their reviver with the percentage; a standing player sees HOLD E TO REVIVE and then REVIVING with progress.

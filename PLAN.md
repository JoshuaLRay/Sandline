...
- **Size:** M
- **Completed 2026-09-21.** The rig contract gains an optional `react(state)` layer: the direction the round came from in the root's own frame, whether it struck the head zone, and a strength. On the skinned soldier the chest twists about the body's axis by the lateral part of that direction (0.4 rad at full strength) and leans away by the frontal part (0.25 rad), composed on the driver's chest with the same undo-if-untouched guard the neck has, and a head-zone hit snaps the neck the same way (0.45 rad) in the one neck write the aim's lean shares; it is applied in the same pass as `hold`, so the two can be called in either order and neither stacks, and a downed soldier shows none and drops the hit it had. `effects.flinch(root, now, hit)` is the entry point: given the shooter's position, the server's point, the damage and the target's feet and hitbox height, it turns the shooter into a root-frame direction, takes the zone from the point's height against `zoneAt` and the fractions in `damage.json`, sizes the reaction by the damage over 50 (floored at 0.3 so a graze reads, capped at 1), and shows the rig what is left of it every frame as exp(-12 x age), closed on the age so 30 and 120 fps agree; under 1% it is withdrawn and the bones are the driver's own bits. A second hit replaces the first with its own direction and size. A rig without `react` (the grey box) gets the T-2.11 translation flinch from the same call, unchanged. main.ts passes the shooter's body on this client (ours, or their replicated mesh), the target's feet and a crouched target's shorter hitbox.
